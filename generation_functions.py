...
from diffusers import DiffusionPipeline, DPMSolverMultistepScheduler, AutoPipelineForImage2Image,EulerDiscreteScheduler
import subprocess
import random
...
def setup(
    ...
    if need_txt2img:
        ...
        if model_id =="playground":
            ...
            pipe = StableDiffusionXLPipeline.from_single_file(base, torch_dtype=torch.float16, variant="fp16").to("cuda")
            ...
            if useTCD:
                ...
                pipe = StableDiffusionXLPipeline.from_single_file(base_model_id, torch_dtype=torch.float16, variant="fp16").to('cuda')
        ...
        if need_img2img:
            ...
            if upscale_model is not None:
                ...
                elif upscale_model.endswith(".ckpt") or model_id.endswith(".safetensors"):
...
def generate_image(prompt, prompt_suffix="", width=512, height=512,
    ...
    if ref_image is not None:
        ...
        if useTCD:
            ...
            images=pipe(prompt, num_inference_steps=num_lightning_steps, guidance_scale=0,width=width,height=height).images
...
def generate_image_url(prompt, prompt_suffix="", width=512, height=512,
                   n_prompt="cropped, collage, composite, (deformed, distorted, disfigured:1.3), poorly drawn, bad anatomy, wrong anatomy, extra limb, missing limb, floating limbs, (mutated hands and fingers:1.4), disconnected limbs, mutation, mutated, ugly, disgusting, blurry, amputation",
                   num_inference_steps=15, batch_size=1,
                   ref_image=None,
                   style_fidelity=1.0,
    ...
    image = generate_image(prompt, prompt_suffix, width, height, n_prompt, num_inference_steps, batch_size, ref_image, style_fidelity, attention_auto_machine_weight, gn_auto_machine_weight, ref_image_scale, clip_skip, cfg_scale)
