...
def generateVideo(project_id, width=512, height=512):
    ...
    #redo one more time but only on elements that have 'priority' >=1
    filtered_elements = [element for element in elements if 'priority' in element and int(element['priority']) >= 1]
    
    #sort by priority (this will cause higher priority elements to be rendered on top)
    filtered_elements = sorted(filtered_elements, key=lambda e: e['priority'])
    
    for element in filtered_elements:
        
        #print('element',element,element['priority'])
...
def generate_video():
    ...
    print('generating movie',project_id)
    url =generateVideo(project_id, width=args.image_sizes[0], height=args.image_sizes[1])
    #prepend / to url
    url = "/" + url
...
def index():
    return redirect("/static/listProjects.html")
...
if __name__ == "__main__":
    ...
    # Add the arguments
    parser.add_argument("--image_sizes", nargs=2, type=int, default=[512, 512], help="The dimensions of the image.")
    
    #--comfy (store true)
    parser.add_argument("--comfy", dest="comfy", action="store_true", help="Enable comfy mode.")
    
    #--comfy_yaml_img default="flux_fp8_comfy.yaml"
    parser.add_argument("--comfy_yaml_img", default="flux_fp8_comfy.yaml", help="The yaml file to use for comfy mode.")
    ...
    # Now you can access the image sizes with args.image_sizes
